        ...
        company_info.update(
            {
                ...
                "EPS Growth (yoy)": (
                    f"{info.get('earningsGrowth', 0) * 100:.2f}%"
...
def generate_company_summary(company_data, stock_history):
    ...
    try:
        ...
        pe_ratio = (
            float(company_data["Trailing P/E"])
            if company_data["Trailing P/E"] != 0
            else 0
        )
        revenue_growth = float(
            company_data["Quarterly Revenue Growth  (yoy)"].strip("%")
        )
        ...
        if company_data["Sector"] != "N/A":
            insights.append(
                f"Operating in {company_data['Sector']} sector, {company_data['Industry']} industry"
            )
        ...
        if price_change_pct > 20:
            insights.append(
                f"Strong stock performance (+{price_change_pct:.1f}% past year)"
            )
        elif price_change_pct > 0:
            insights.append(
                f"Positive stock performance (+{price_change_pct:.1f}% past year)"
            )
        else:
            insights.append(
                f"Declining stock value ({price_change_pct:.1f}% past year)"
            )
...
def analyze_company(symbol):
    try:
        ...
        if not company_data:
            error_msg = (
                f"Unable to fetch data for {symbol}. Please verify the company symbol."
            )
            if target_lang != "en":
                error_msg = translate_text(error_msg, target_lang)
        ...
        analysis = {
            ...
            "key_metrics": {
                "profitability": (
                    float(company_data["Profit Margin"].strip("%"))
                    if isinstance(company_data["Profit Margin"], str)
                    else 0
                ),
                "pe_ratio": (
                    float(company_data["Trailing P/E"])
                    if company_data["Trailing P/E"] != "N/A"
                    else 0
                ),
                "market_cap": company_data["Market Cap"],
                "revenue_growth": (
                    float(company_data["Quarterly Revenue Growth  (yoy)"].strip("%"))
                    if isinstance(company_data["Quarterly Revenue Growth  (yoy)"], str)
                    else 0
                ),
            },
            "summary": summary,
